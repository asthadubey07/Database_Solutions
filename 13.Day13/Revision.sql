...
--Try catch
	begin try
	declare @num1 int= 5/0;
	end try
	begin catch
	throw 5500000 , 'please dont use 0 in denominater',3;
	end catch
...
alter table EmployeesAudit alter column Operation varchar(50);
INSERT INTO dbo.Employees_infoo ( EmployeeName ,EmployeeAddress ,MonthSalary )
values( 'Mark Smith', 'Ocean Dr 1234', 10000),('Joe Wright', 'Evergreen 1234', 10000),
('John Doe', 'International Dr 1234', 10000),('Peter Rodriguez', '74 Street 1234', 10000)


CREATE TRIGGER TR_Audit_Employees_insert ON dbo.Employees_infoo
FOR INSERT
AS
DECLARE @login_name VARCHAR(128)
INSERT  INTO dbo.EmployeesAudit ( EmployeeID , EmployeeName ,EmployeeAddress ,MonthSalary , ModifiedDate ,Operation )
SELECT  I.EmployeeID ,I.EmployeeName ,I.EmployeeAddress , I.MonthSalary , GETDATE() ,'Insert' FROM    Inserted I

CREATE TRIGGER TR_Audit_Employees_DELETE ON dbo.Employees_infoo
FOR DELETE
AS
INSERT  INTO dbo.EmployeesAudit ( EmployeeID , EmployeeName ,EmployeeAddress ,MonthSalary ,  ModifiedDate ,Operation )
SELECT  D.EmployeeID ,D.EmployeeName ,D.EmployeeAddress , D.MonthSalary , GETDATE() ,'Deleate' FROM   Deleted D

CREATE TRIGGER TR_Audit_Employees_UPDATE ON dbo.Employees_infoo
FOR UPDATE
AS
INSERT  INTO dbo.EmployeesAudit ( EmployeeID , EmployeeName ,EmployeeAddress ,MonthSalary , ModifiedDate ,Operation )
SELECT  U.EmployeeID ,U.EmployeeName ,U.EmployeeAddress , U.MonthSalary , GETDATE() ,'UPDATE' FROM   inserted u

DELETE
FROM Employees_infoo
WHERE EmployeeID = 8;

select * from Employees_infoo;
select * from EmployeesAudit;

UPDATE Employees_infoo
SET EmployeeName = 'astha'
WHERE EmployeeID = 5;
...
Insert into Employees2 values (@Name, @Email, @Dept)
Insert into Employees3 values (@Name, @Email, @Dept)
...
CREATE PROCEDURE Insert_Customers7 (@name varchar(50),@age int,@number bigint,@address varchar(200))
as
insert into Customers7 values (@name,@age,@number,@address)
go
EXEC Insert_Customers7 @name ='pankal',@age=50,@number=1234567890,@address ='gygfiyi';




--How to modify stored procedure
ALTER PROCEDURE Insert_Customers7 (@name varchar(200),@age int,@number bigint,@address varchar(200))
as
insert into Customers7 values (@name,@age,@number,@address)
go
EXEC Insert_Customers7 @name ='pankal',@age=50,@number=1234567890,@address ='gygfiyi';
...
--How to use prebuilt function
prebuilt functions in SQL Server are the database objects that contains a set of SQL statements to perform a specific task. like getdate(),trim()
SELECT DATEADD(month, 5, '2023/09/20') AS DateAdd;
...
SELECT AVG(Cus_age) AS avgage FROM Customers7; 
...
SELECT LCASE(Cus_name) FROM Customers7
...
SELECT CONCAT(Cus_name,Cus_age) AS info
FROM Customers7;
...
SELECT TRIM('A   s   t   h   a                ') ;



--Length
--count the length of given expression
SELECT LEN('Astha Dwivedi');
...
SELECT DATEADD(month, 3, '2023/09/20') AS DateAdd;



--DateDiff
--Returns the number of days between two date values
SELECT DATEDIFF(month, '2023/09/19', '2016/08/03') AS DateDiff;



--DateFormats
--	Formats a date
SELECT DATEFROMPARTS(2023,09,20) AS DateFromParts;



--DateName
--Returns the name for a given date
SELECT DATENAME(year, '2023/09/20') AS DatePartString;
...
--How to create stored prodecure with parameter
CREATE PROCEDURE SelectAllCustomers @id int
AS
SELECT * FROM Customers7 WHERE  Cus_Id = @id 


--Casting
--convert float into int
select cast (4.77 as int)

-- convert string into datetime
select cast('2017-08-25' as datetime)

--IIF
SELECT Cus_Id,Cus_name,IIF(age>50, 'old', 'not old') as result, ISNUMERIC(number) as phone
FROM Customers7;


--how to create function
create function add_number(@first_number int ,@second_number int)
returns varchar(200)
as
begin
...
select dbo.add_number(2,3)


--How to modify
ALTER  function add_number(@first_number int ,@second_number int)
returns int 
as
begin
return @first_number +  @second_number
...
CREATE FUNCTION find_Row ( @Id int )
  RETURNS table
  as
   return select * from Customers7 where Cus_Id=@Id

SELECT * FROM find_Row(2)



--Multi statement function
create function show_table()
returns table
as
begin
select * from Customers7
...
--CTE
--to store the data from query we use with key word
with savedata as (select * from Customers7)
--select * from savedata;
select Cus_Id from savedata


---over clause 
select Cus_name,Cus_age,count(*) over(partition by age) from Customers7;
...
select Customer_Order.Order_id ,Customer.Customer_name from Customer inner join Customer_Order on Customer.Customer_Id = Customer_Order.cus_id;
select * from Customer inner join Customer_Order on Customer.Customer_Id = Customer_Order.cus_id;
...
select Customer_Order.Order_id ,Customer.Customer_name from Customer Left join Customer_Order on Customer.Customer_Id = Customer_Order.cus_id;
select * from Customer Left join Customer_Order on Customer.Customer_Id = Customer_Order.cus_id;
...
select Customer_Order.Order_id ,Customer.Customer_name from Customer right join Customer_Order on Customer.Customer_Id = Customer_Order.cus_id;
select * from Customer right join Customer_Order on Customer.Customer_Id = Customer_Order.cus_id;
...
select Customer_Order.Order_id ,Customer.Customer_name from Customer Outer join Customer_Order on Customer.Customer_Id = Customer_Order.cus_id;
select * from Customer Outer join Customer_Order on Customer.Customer_Id = Customer_Order.cus_id;
